...
// --- Kumpulan Ikon SVG (agar kode utama lebih bersih) ---
...
const SwitchIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-400 mx-2 cursor-pointer hover:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
const SearchWidget = () => {
  ...
  // State untuk form pencarian pesawat
  ...
  // State untuk form pencarian kereta
  const [trainData, setTrainData] = useState({
    origin: '',
    destination: '',
    departureDate: '',
    passengers: '', // Ditambahkan
  });

  // Generic handler untuk mengubah state form
  ...
  const handleSearchSubmit = () => {
    if (activeTab === 'pesawat') {
      const { origin, destination, departureDate } = flightData;
      if (!origin || !destination || !departureDate) {
        alert('Harap lengkapi asal, tujuan, dan tanggal pergi.');
        return;
      }
      // Membuat query string dan navigasi
      const query = new URLSearchParams(flightData).toString();
      router.push(`/search/flights?${query}`);
    } else {
      const { origin, destination, departureDate } = trainData;
       if (!origin || !destination || !departureDate) {
        alert('Harap lengkapi stasiun asal, tujuan, dan tanggal berangkat.');
        return;
      }
      const query = new URLSearchParams(trainData).toString();
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <div className="md:col-span-2 lg:col-span-2">
            ...
            <div className="flex items-center bg-white p-2 rounded-md border border-gray-200 mt-1">
              <input type="text" name="origin" value={flightData.origin} onChange={handleFlightChange} placeholder="Kota atau Bandara Asal" className="bg-transparent w-full focus:outline-none placeholder-gray-400 text-gray-900" />
              <SwitchIcon />
              <input type="text" name="destination" value={flightData.destination} onChange={handleFlightChange} placeholder="Kota atau Bandara Tujuan" className="bg-transparent w-full focus:outline-none text-right placeholder-gray-400 text-gray-900" />
            </div>
          </div>
          {/* Input Tanggal */}
          <div>
            ...
            <div className="flex items-center bg-white p-2 rounded-md border border-gray-200 mt-1">
              <CalendarIcon />
              <input type="date" name="departureDate" value={flightData.departureDate} onChange={handleFlightChange} placeholder="Pilih Tanggal" className="bg-transparent w-full focus:outline-none ml-2 text-gray-900"/>
            </div>
          </div>
          {/* Input Penumpang */}
          <div>
            ...
            <div className="flex items-center bg-white p-2 rounded-md border border-gray-200 mt-1">
                <UserIcon />
                <input type="text" name="passengers" value={flightData.passengers} onChange={handleFlightChange} placeholder="2 Dewasa, 1 Anak" className="bg-transparent w-full focus:outline-none ml-2 placeholder-gray-400 text-gray-900" />
            </div>
          </div>
        </div>
      );
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="flex items-center bg-white p-2 rounded-md border border-gray-200 mt-1">
              <input type="text" name="origin" value={trainData.origin} onChange={handleTrainChange} placeholder="Stasiun Asal" className="bg-transparent w-full focus:outline-none placeholder-gray-400 text-gray-900" />
            </div>
          </div>
          {/* Input Tujuan */}
          <div className="lg:col-span-1">
            ...
            <div className="flex items-center bg-white p-2 rounded-md border border-gray-200 mt-1">
              <input type="text" name="destination" value={trainData.destination} onChange={handleTrainChange} placeholder="Stasiun Tujuan" className="bg-transparent w-full focus:outline-none placeholder-gray-400 text-gray-900" />
            </div>
          </div>
          {/* Input Tanggal */}
          <div className="lg:col-span-1">
            <label className="text-gray-800 text-sm font-medium">Tanggal Berangkat</label>
            <div className="flex items-center bg-white p-2 rounded-md border border-gray-200 mt-1">
                <CalendarIcon />
                <input type="date" name="departureDate" value={trainData.departureDate} onChange={handleTrainChange} placeholder="Pilih Tanggal" className="bg-transparent w-full focus:outline-none ml-2 text-gray-900"/>
            </div>
          </div>
           {/* Input Penumpang (Ditambahkan) */}
           <div className="lg:col-span-1">
            <label className="text-gray-800 text-sm font-medium">Penumpang</label>
            <div className="flex items-center bg-white p-2 rounded-md border border-gray-200 mt-1">
                <UserIcon />
                <input type="text" name="passengers" value={trainData.passengers} onChange={handleTrainChange} placeholder="1 Dewasa" className="bg-transparent w-full focus:outline-none ml-2 placeholder-gray-400 text-gray-900" />
            </div>
          </div>
      </div>
    );
...
export default function Home() {
  ...
  return (
    <>
      {/* Hero Section */}
      <section 
        className="relative flex items-center justify-center h-[60vh] min-h-[500px] bg-cover bg-center text-white" 
