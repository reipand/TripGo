...
const TrainTicketCard = ({ train }: { train: Train }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 transition-all duration-300 hover:shadow-xl">
      <Image 
        src={`/images/train-logo-${train.transportations.nama_transportasi.toLowerCase().replace(/\s+/g, '-')}.png`} 
        alt={`${train.transportations.nama_transportasi} logo`} 
        width={96} 
        height={96} 
        className="w-24 h-auto object-contain"
        onError={(e) => { e.currentTarget.src = '/images/train-logo-default.png'; }}
        placeholder="blur"
...
const TrainResults = () => {
  ...
  const [filteredAndSortedTrains, setFilteredAndSortedTrains] = useState<Train[]>([]);
  ...
  useEffect(() => {
    ...
    setFilteredAndSortedTrains(result);
        ...
        <>
          {filteredAndSortedTrains.length > 0 && (
            <p className="text-sm text-gray-500 mb-4">
              Menampilkan {filteredAndSortedTrains.length} kereta
            </p>
          )}
          <div className="space-y-4">
            {filteredAndSortedTrains.length > 0 ? (
